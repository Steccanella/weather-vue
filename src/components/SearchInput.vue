  ...
  searchTerm.timeout = setTimeout(async () => {
    if (searchTerm.query !== '') {
      const res = await fetch(
        `http://api.weatherapi.com/v1/search.json?key=b985c48c6fda42d5a4d111423240903&q=${searchTerm.query}`
        )
...
const getWeather = async (id) => {
  const res = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=b985c48c6fda42d5a4d111423240903&q=id:${id}&days=3&aqi=no&alerts=no`)
  const data = await res.json();
